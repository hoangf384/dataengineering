...
def get_reference_data(spark: SparkSession) -> Tuple[list, list]:
    ...
    jobs_df = (
        ...
        .option(
            "dbtable", "(SELECT id as job_id, campaign_id, group_id FROM job) as tmp"
        )
    ...
    pub_df = (
        ...
        .option(
            "dbtable",
            "(SELECT distinct(id) as publisher_id FROM master_publisher) as tmp",
        )
        .option("driver", "com.mysql.cj.jdbc.Driver")
        .option("user", MYSQL_USER)
        .option("password", MYSQL_PASSWORD)
...
def generate_and_write_batch(
    spark: SparkSession, n_records: int, jobs_list: list, pub_list: list
):
    """
